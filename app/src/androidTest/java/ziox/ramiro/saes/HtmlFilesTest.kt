            ...
            withContext(Dispatchers.Main.immediate) {
                files.forEach { file ->
                    ...
                    runCatching {
                        ...
                        Log.d(
                            "HtmlFilesTest",
                            "File $file processed successfully with ${it.size} classes"
                        )
                    }
                }
            }
    ...
    fun testGradesFiles() = runTest {
        ActivityScenario.launch(AboutActivity::class.java).use { scenario ->
            ...
            withContext(Dispatchers.Main.immediate) {
                files.forEach { file ->
                    val repository =
                        ScheduleWebViewRepository(activity, withTestFile = "grades_errors/$file")
    ...
    fun testProfileFiles() = runTest {
        ActivityScenario.launch(AboutActivity::class.java).use { scenario ->
            ...
            withContext(Dispatchers.Main.immediate) {
                files.forEach { file ->
                    val repository =
                        ScheduleWebViewRepository(activity, withTestFile = "profile_errors/$file")
    ...
    fun testLoginFiles() = runTest {
        ActivityScenario.launch(AboutActivity::class.java).use { scenario ->
            ...
            withContext(Dispatchers.Main.immediate) {
                files.forEach { file ->
                    val repository =
                        ScheduleWebViewRepository(activity, withTestFile = "login_errors/$file")
